...
public class FilterSubProperties<PT extends OntProperty> implements PropertyFilter<PT> {

    private final PT baseP;
    private final Class<PT> cls;

    private FilterSubProperties(final PT baseP, final Class<PT> cls) {
        this.baseP = baseP;
        this.cls = cls;
    }

    public static <PT extends OntProperty> Function<PT, PropertyFilter<PT>> factory(final Class<PT> cls) {
        return (final PT baseP) -> new FilterSubProperties<PT>(baseP, cls);
    }
    ...
    public boolean accept(final PT prop) {
        return prop.hasSubProperty(this.baseP, true);
    }
}
